...
export default function HeroSection() {
  ...
  return (
    <section id="hero" className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div
        className="absolute bottom-20 right-10 w-16 h-16 bg-secondary/30 rounded-full animate-float"
        style={{ animationDelay: "2s" }}
      ></div>
      <div
        className="absolute top-1/2 left-20 w-12 h-12 bg-accent/25 rounded-full animate-float"
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-muted-foreground mb-6">{t("hero.title")}</p>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{t("hero.description")}</p>
          </div>

          {/* CTA Buttons */}
          <div
            className={`mb-12 transition-all duration-1000 delay-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
          >
